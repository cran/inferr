test_that("output from anova_split matches the expected result", {
  ...
  expect_equal(k$cyl, c(4, 6, 8))
  expect_equal(k$length, c(11, 7, 14))
  expect_equal(round(k$mean, 2), c(26.66, 19.74, 15.100))
  expect_equal(round(k$var, 2), c(20.34, 2.11, 6.55))
  expect_equal(round(k$sd, 2), c(4.51, 1.45, 2.56))
  expect_equal(round(k$sst, 2), c(475.25, 0.85, 348.69))
  expect_equal(round(k$sse, 2), c(203.39, 12.68, 85.2))
})
...
test_that("output from text formatting matches the expected result", {
  expect_equal(fg(3, 10), "    3     ")
  expect_equal(fs(), "  ")
  expect_equal(fl(3, 10), "3         ")
  expect_equal(fc(3, 10), "    3     ")
  expect_equal(fk(3, 10), "     3.000")
  expect_equal(fw(3, 10), "    3     ")
  expect_equal(fn(3, 10), "    3     ")
  expect_equal(formats(), "    ")
  expect_equal(formatter_t(3, 10), "    3     ")
  expect_equal(format_cil(3, 10), "    3     ")
  expect_equal(format_ciu(3, 10), "    3     ")
  expect_equal(formats_t(), "  ")
  expect_equal(formatter_pair(3, 6), " 3.00 ")
})
...
test_that("output from serr matches the expected result", {
  k <- ifr_mcnemar_test(matrix(c(135, 18, 21, 26), nrow = 2))
...
test_that("output from tibble_stats matches the expected result", {
  ...
  expect_equal(k[[1]], c(4, 6, 8))
  expect_equal(k[[2]], c(11, 7, 14))
  expect_equal(round(k[[3]], 3), c(26.664, 19.743, 15.100))
  expect_equal(round(k[[4]], 3), c(20.339, 2.113, 6.554))
  expect_equal(round(k[[5]], 3), c(4.510, 1.454, 2.560))
  expect_equal(round(k[[6]], 3), c(1.360, 0.549, 0.684))
})
...
test_that("output from paired_data and paired_stats matches the expected result", {
  ...
  expect_equal(k[[1]], c(52.230, 52.775, -0.545))
  expect_equal(round(k[[2]], 2), c(10.25, 9.48, 8.89))
  expect_equal(round(k[[3]], 2), c(0.72, 0.67, 0.63))
})
...
test_that("output from conf_int_t matches the expected output", {
  ...
  n <- length(hsb$read)
  expect_equal(round(conf_int_t(b[[1, 1]], b[[2, 1]], n, alpha = 0.025), 2), c(43.80, 60.66))
})
...
test_that("output from conf_int_p matches the expected result", {
  expect_equal(round(conf_int_p(10, 3.6), 3), c(2.944, 17.056))
})
...
test_that("output from sums and coch matches the expected result", {
  ...
  k <- sums(data)
  expect_equal(k[[1]], c(x = 10, y = 13, z = 13))
  expect_equal(k[[2]], 438)
  expect_equal(k[[3]], c(2, 1, 3, 2, 3, 3, 2, 0, 3, 1, 1, 1, 2, 2, 1, 3, 3, 1, 1, 1))
  expect_equal(k[[4]], 82)
